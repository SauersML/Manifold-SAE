...
| `../f4_ln_sphere_pullback_2026-07-04/` | LN-sphere ambient (F4-D1) + behavior-first pullback / transport (F4-D2), Qwen3-8B weekday | **The task-invariant object is the pullback from output geometry, not the embedded set.** Mirror pair on the SAME feature: the input-encoding probe ("Today is Monday") has weekday in the *activation* (clean sort) but not the behavior (template-dominated, same-template KL 0.19 vs same-day 1.48); the output-computation probe ("The day after Monday is __", pred-acc 0.71) flips — behavior carries the answer (**order-purity 1.00, weekday-KL calibration r² 0.63 high-confidence** vs activation-first 0.017), while the activation carries the arithmetic (order-purity 0.41). **Transport without refit:** the behavior coordinate is fit once and holds at L11/L18/L23 (order-purity 1.00, stable ~14% pullback EV) while the activation circle refits and drifts (cross-layer corr 0.28–0.42, purity 0.47→0.41→0.36) — the atom-level image of the `transport/` metric result (J-propagation to 0.1%). D2 synthetic (behavior carries feature by construction) is the first direct pullback-architecture evidence: behavior-first calibrates KL **tighter** (r² 0.94 vs 0.86), circles agree (corr 0.95), pullback explains 99% of activation. **D1:** structured-residual can't cover LN structure (θ-rotating radial nuisance) → `ln_sphere_project` + green Rust test; flat spurious curvature 0.008→0.504 vs sphere-decoder machine-invariant 1.9e-14. Figure + numbers + repro in the dir's README. |
