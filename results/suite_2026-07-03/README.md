...
| `frontiers/` | compute-matched EV/L0 + bits-per-token frontiers: curved refinement vs TopK/block linear | **Real 35B L17 block lane DONE** (EVs lifted from frame-health; biased-slice + tier0-mean caveats stamped): EV 0.707/0.906/0.990 at K∈{4k,16k,32k}; at matched distortion the massive-K dictionary dominates description length (~7k bits/token @16k blocks). Full analytic FLOP accounting (train+inference) + MDL bits (combinatorial + support-entropy H(S)). **Synthetic curved frontier = honest NEGATIVE (falsified, gam#2132)**: the multi-atom `sae_manifold_fit` dictionary co-collapses on planted circles (held-out EV 0.23–0.26, *decreasing* in K, below a trivial linear-PCA 0.55–0.73 on identical data; curved underperforms even same-lane linear) — a fair curved-wins frontier can't be drawn with the current solver. Harness + FLOP/MDL accounting complete & correct; they produce the frontier once the solver clears the linear ceiling. |
