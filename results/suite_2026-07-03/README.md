...
| `frontiers/` | compute-matched EV/L0 + bits-per-token frontiers: curved refinement vs TopK/block linear | **Real 35B L17 block lane DONE**: EV 0.707/0.906/0.990 at K∈{4k,16k,32k}; at matched distortion the massive-K dictionary dominates description length (~7k bits/token @16k blocks) — the regime a fewer-atom curved code would win, but curved REML is a small-K tool (>6min/fit at p=256) so the two lanes are **complementary**, reported honestly. Full analytic FLOP accounting (train+inference) in `FLOP_ACCOUNTING.md`. **Synthetic massive-p frontier** (curved vs same-lane linear, heavy-tailed firing, p→1024) in flight on MSI (jobs 12501080/209/212); verdicts via `frontier_analyze.py`. |
