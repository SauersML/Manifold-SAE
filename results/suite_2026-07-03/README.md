...
| `transport/` | per-layer chart fits L11–L23 + carried-vs-rotated metric transport (JVP arm), Qwen3-8B weekday circle | **Carried in correspondence, re-encoded in geometry.** Every hop preserves the same-day↔same-phase map (degree 1, degree-conc 0.85–0.99) but tilts the circle's 2-plane ~17–42°/hop (net 44–53° L11→L23, never orthogonal) and is non-isometric (defect O(1)) → all 12 hops classify **compute**, not carry. Shuffled-day null **PASSES**: real degree-conc 0.949 vs null 0.156±0.055 = **14.4σ**. **JVP arm (headline): the L18 output-Fisher metric propagated by the frozen-model Jacobian down 7 layers to L11 matches the metric harvested directly at L11 to <0.1%** (pred log-corr 0.9995; slope/R²/ratio equal to 3 sig figs) — harvest once, propagate anywhere. The one L20→L21 degree-2/defect-4.56 outlier is **resolved (XPORT+TOPO joint): a chart-parametrization artifact at L20, not a topological transition** — degree-2 only at rank 8 (→1 at rank 16/24), localized to L20-endpoint hops; fit-free persistent homology finds an intact single loop (b0=1, one dominant b1) at both L20 and L21 across a 500× bootstrap (n=210); the JVP metric also rides that hop at corr 1.0. Full numbers + figures + repro scripts in `transport/report.md`. |
| `../f4_ln_sphere_pullback_2026-07-04/` | LN-sphere ambient (F4-D1) + behavior-first pullback / transport (F4-D2), Qwen3-8B weekday | **The task-invariant object is the pullback from output geometry, not the embedded set.** Mirror pair on the SAME feature: the input-encoding probe ("Today is Monday") has weekday in the *activation* (clean sort) but not the behavior (template-dominated, same-template KL 0.19 vs same-day 1.48); the output-computation probe ("The day after Monday is __", pred-acc 0.71) flips — behavior carries the answer (**order-purity 1.00**) while the activation carries the arithmetic (order-purity 0.41). **Transport without refit:** the behavior coordinate is fit once and holds at L11/L18/L23 (order-purity 1.00 — a fitter-robust coordinate-level claim) while the activation circle refits/drifts (cross-layer corr 0.28–0.42, purity 0.47→0.41→0.36). **SPEC-compliance caveat (in progress):** D2 circle fits were numpy-sourced; re-running them through `gamfit.sae_manifold_fit` RETRACTS the "behavior-first calibrates KL tighter" claim (synthetic re-fit TIES: r² 0.63 vs 0.64, not 0.94 vs 0.86). Surviving = order/geometry (mirror pair, order-purity, latent recovery, pullback EV 0.92); a direct JVP test of the stronger "tangent plane composes with J" claim is a NEGATIVE (J·plane stays 73–80° off; the invariance is the coordinate ordering, the metric-length invariance is `transport/`'s 0.1% result). Numbers pending newest-gamfit re-derivation. **D1:** structured-residual can't cover LN structure (θ-rotating radial nuisance) → `ln_sphere_project` + green Rust test; flat spurious curvature 0.008→0.504 vs sphere-decoder machine-invariant 1.9e-14. Figure + numbers + repro in the dir's README. |
| `frontiers/` | compute-matched EV/L0 + bits-per-token frontiers: curved refinement vs TopK/block linear | **Real 35B L17 block lane DONE** (EVs lifted from frame-health; biased-slice + tier0-mean caveats stamped): EV 0.707/0.906/0.990 at K∈{4k,16k,32k}; at matched distortion the massive-K dictionary dominates description length (~7k bits/token @16k blocks). Full analytic FLOP accounting (train+inference) + MDL bits (combinatorial + support-entropy H(S)). **Synthetic HEADLINE (in-frame cascade arm): SAE + evidence-priced curved refinement CANNOT lose EV and wins bits** — drawn with the shipped in-frame path (`inframe_frontier.py`, numpy mirror of `inframe_curved.rs`), 5 seeds × p∈{48,256,1024,2048}: ΔEV +0.72…+0.85 (curved 0.83–0.99 vs linear 0.11–0.14), Δbits strongly negative, border M·r not M·p (shrink up to 64×) so it's also cheaper — verdict True/True. **Why the cascade, not the cold joint (gam#2132)**: the deprecated multi-atom `sae_manifold_fit` dictionary co-collapses on the same planted data (held-out EV 0.23–0.26, decreasing in K, below trivial linear-PCA) — the honest "why this arm" row, not a dead end. |
| `premise/` | held-out paired-deviance premise instrument + slow-feature atlas | **Curvature's dividend is real in activations, inert in behavior.** The calibration-free test (line vs circle, leave-template-out, paired per-row deviance, robust sign test; falsification passed — Gaussian surrogate flat, sign-flip null calibrated): **behaviorally curvature NEVER pays at 8B·L18** — weekday **COSTS** (sign-test p≈1e-3, line wins ~70% of held-out rows; the closed circle can't extrapolate a held-out point the line can), month/sycophancy/hedging flat honest negatives. But **in activation space the circle significantly reduces held-out reconstruction for the graded features** (sycophancy median +648, hedging +214, sign-test p<0.003, ~70% rows) — real geometry landing in behaviorally-inert directions. The cyclic calendar features don't cross-validate even in activations → the topology races' circle preference is **post-selection optimism**. This sharpens the crown: the metric buys dose *forecasting*, not per-row behavioral *likelihood*. **Slow-feature atlas:** the subtracted PerContextMean **IS a modeled feature** — feature-of-origin perfectly recoverable from its geometry (1-NN LOO 1.00 vs Gaussian-null 0.27–0.31, perm p=2e-4), dominated by one common-mode axis (PC1 0.998 @8B). 35B replication (color, weekday, month) **BLOCKED by a gamfit OOS-predict basis bug** (filed); day-of-month + joint month×day torus (pair-κ) in flight. Full numbers/figures/repro in `premise/report.md`. |
...
| **curvature has NO held-out behavioral dividend; weekday it COSTS** | `premise/report.md` | The calibration-free held-out paired-deviance instrument (line vs circle, leave-template-out) finds curvature never lowers held-out *behavioral* (output-Fisher, nats) deviance at 8B·L18: month/sycophancy/hedging flat (sign-test n.s.), and **weekday curvature COSTS** (sign-test p≈1e-3, line wins ~70% of held-out rows — the closed circle mis-projects held-out points a line extrapolates). Direct held-out confirmation of the "linear+Fisher not breaking / curvature-in-nats unproven" miss below. The circle *does* win in raw activation space for the graded features (real geometry) — but behaviorally inert. |
| **topology-race circle preference is post-selection optimism (cyclic features)** | `premise/report.md` | weekday/month circles win the in-sample topology race but do **not** cross-validate: leave-template-out, the raw held-out paired-deviance median is n.s./negative (circle no better than line on unseen templates). Not a demeaning or complexity-charge artifact (raw held-out deviance, identical per-prompt demeaning). |
| **35B premise replication blocked by gamfit OOS-predict bug** | `premise/report.md`; gam issue (filed) | `sae_manifold_predict_oos: decoder_blocks[0] has M=2 but rebuilt basis has M=3` on 35B circle held-out reconstruct; 35B color fit additionally grinds. Crude polyline fallback (corr ~0.54 vs true reconstruct) exists but is NOT published. 35B color/weekday/month deferred to the bugfix. |
