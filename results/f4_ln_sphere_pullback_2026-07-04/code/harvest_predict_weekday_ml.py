...
OUT = f"{R}/dose_qwen8b_out/predict_weekday_multilayer.npz"
LAYERS = [11, 18, 23]
TOPK = 256
...
def main():
    ...
    with torch.no_grad():
        for i, p in enumerate(prompts):
            ...
            for _l in LAYERS: acts.setdefault(_l, []).append(out.hidden_states[_l][0, -1, :].float().numpy())
            logits_last.append(out.logits[0, -1, :].float().numpy())
            if (i + 1) % 14 == 0:
                print(f"  {i+1}/{n} {time.time()-t0:.0f}s", flush=True)
    XL = {f"X_last_L{l}": np.stack(v, 0).astype(np.float64) for l, v in acts.items()}
