...
- **Stage 1 (guard surgery) — LANDED** (gam `8f14b403d`): absolute-degeneracy
  collapse floor, no iteration-0 reseed. **No post-S1 numbers exist yet** — the
  first real post-surgery evidence will be A1's four-arm rerun on the rebuilt
  `venv_fable`; until those land, every live-SAC cell stays *pending post-S1
  rerun*. (The W7 REML in-sample EVs below — weekday 0.525 / month 0.243 / year
  0.335 / color 0.627 — were produced on the PRE-fix build, gamfit 0.1.247 PyPI,
  and are NOT S1 evidence.)
...
Stage 1 guard surgery (landed) targets exactly this, but **no post-S1 numbers
exist yet**: the earlier NONCONVERGENCE/OOM attempts differed from later runs in
venue and data-shape (laptop OOM reaper; chart_transfer's d=5120 ambient
residuals), not in build, so they do not form a clean before/after. The first
post-surgery evidence will be A1's four-arm rerun on the rebuilt `venv_fable`.
...
linear-1PC. REML `sae_manifold_fit` in-sample cross-checks (0.24-0.63) were run on
the PRE-S1 build (gamfit 0.1.247), so they are not post-S1 evidence. Honest
