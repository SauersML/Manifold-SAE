"""Synthetic 2D recovery v2 — ablate why the 2D atom arch loses to 2×1D.

From docs/findings.md: at matched parameter budget on planted 2D grids,
two 1D atoms beat one 2D atom on every grid (per-grid Spearman² 0.34-0.49
vs 0.16-0.25). The hypothesis is that the 2D arch is under-regularized:
no isotropy loss, no per-axis coverage, no ortho, and a shallow encoder
that can't invert the planted curvature.

This script runs an ablation grid:
  v0  baseline (current synthetic_2d_recovery.py recipe)
  v1  + isotropy (push intrinsic_dim_ratio → 1 on firing atoms)
  v2  + ortho (push directions apart across atoms)
  v3  + per-axis coverage (firing positions spread over [0,1] on BOTH axes)
  v4  deeper encoder (2× MLP)
  v5  lower n_basis K=4 (per-axis), per the "lower-K unlocks atom utilization"
       finding from 1D
  v6  all combined
  v7  oracle-encoder baseline (skip encoder, plug GT t,s in — answers
       whether the basis+penalty CAN fit the planted manifolds)

For each variant we report:
  * EV (1 − MSE/Var)
  * alive atom count
  * per-grid best-atom score (mean Spearman² of (t̂,ŝ) vs (t,s) restricted
    to that grid's firing tokens)
  * intrinsic_dim_ratio distribution (median, frac > 0.5)

If a variant pushes per-grid score from 0.20 → 0.50, that's a real
architectural improvement. If NONE of them do — even v7 — then the
planted manifold may not be recoverable at this budget and the lever to
move is at the data-generation level (lower curvature, more atoms, etc).
"""
...
class Config:
    ...
    output_dir: str = os.environ.get("MANIFOLD_SAE_OUTPUT_DIR", "/content/runs/SYNTHETIC_2D_V2")


def plant_2d_manifolds(cfg: Config) -> dict:
    rng = np.random.default_rng(cfg.seed)
    D, G, K_grid = cfg.d_ambient, cfg.n_grids, cfg.grid_size  # noqa: F841
    bases, curvatures = [], []
    for _ in range(G):
    ...
    return {
        "X": torch.from_numpy(X), "active": active,
...
def _ortho_loss(dirs: torch.Tensor) -> torch.Tensor:
    """Push directions apart across atoms. dirs: (F, D, R)."""
    F, D, R = dirs.shape
    # flatten each atom's subspace and take pairwise cos² off-diagonal.
    flat = dirs.reshape(F, D * R)
...
def _replace_encoder_with_deeper(sae) -> None:
    ...
    F = enc.n_features
    H = enc.hidden_dim
    # Replace fc1 with a 2-layer MLP
...
def train_2d_variant(cfg: Config, X: torch.Tensor, device: torch.device,
                     variant: str, data: dict) -> dict:
    ...
    for step in range(cfg.n_steps):
        ...
        if use_iso:
            # Penalize log-ratio: |log(ratio)| -> 0 means ratio=1 (isotropic).
            # Restrict to atoms with non-trivial firing so dead atoms don't dominate.
            fire = (out.amplitudes > 1e-6).float().mean(dim=0)        # (F,)
            alive_mask = (fire > 0.05).float()
            iso_term = (torch.log(out.intrinsic_dim_ratio.clamp(min=0.05)).abs() * alive_mask).sum() / (alive_mask.sum() + 1e-6)
            loss = loss + 5e-3 * iso_term
        if use_ortho:
            loss = loss + 1e-3 * _ortho_loss(out.directions)
        if use_cov:
            cov_t = _coverage_kl(out.positions_t, out.amplitudes)
            cov_s = _coverage_kl(out.positions_s, out.amplitudes)
            loss = loss + 1e-2 * (cov_t + cov_s)
        loss.backward()
...
def main() -> int:
    ...
    for variant in cfg.variants:
        ...
        try:
            obj = train_2d_variant(cfg, data["X"], device, variant, data)
            sae = obj["sae"]
            report = evaluate_2d(cfg, sae, data, device)
            ...
            # Save snapshot per variant
            (out_dir / f"{variant}_results.json").write_text(json.dumps(report, indent=2, default=float))
    ...
    ranked = sorted(
        ((v, r) for v, r in all_results.items() if "mean_per_grid" in r),
        key=lambda kv: -kv[1]["mean_per_grid"]
    )
    print(f"{'variant':22} {'EV':>6} {'alive':>6} {'per_grid':>9} {'frac2d':>7}", flush=True)
    for v, r in ranked:
        print(f"{v:22} {r['ev']:6.3f} {r['alive']:6d} {r['mean_per_grid']:9.3f} {r['ratio_frac_2d']:7.2f}", flush=True)
