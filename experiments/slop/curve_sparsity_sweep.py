...
def train_curve(cfg: Config, X: torch.Tensor, device, top_k: int):
    ...
    sae_cfg = ManifoldSAEConfig(
        input_dim=X.shape[1], n_atoms=cfg.sae_F, n_basis_per_atom=cfg.sae_n_basis,
    ...
    for step in range(cfg.n_steps_curve):
        ...
        opt.step()
        if step % 400 == 0:
            print(f"      [crv top_k={top_k} step {step:4d}] mse={F_nn.mse_loss(out.x_hat,batch).item():.4e}", flush=True)
    sae.eval()
    sae.fit(X[:2048])
    sae.lock_snapshot()
    return sae
