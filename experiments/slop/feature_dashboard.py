...
def main() -> int:
    ...
    # All flags fall back to env vars so a Heimdall submitter can drive
    # the experiment without CLI args.
    parser.add_argument("--checkpoint",
                        default=os.environ.get("MSAE_CHECKPOINT"),
                        help="curve SAE .pt path (or env MSAE_CHECKPOINT)")
    parser.add_argument("--model",
                        default=os.environ.get("MSAE_MODEL", "Qwen/Qwen2.5-0.5B"))
    parser.add_argument("--layer", type=int,
                        default=int(os.environ.get("MSAE_LAYER", "12")))
    parser.add_argument("--n-tokens", type=int,
                        default=int(os.environ.get("MSAE_DASH_N_TOKENS", "20000")))
    parser.add_argument("--top-k-per-atom", type=int,
                        default=int(os.environ.get("MSAE_DASH_TOPK", "30")))
    parser.add_argument("--output",
                        default=os.environ.get(
                            "MSAE_DASH_OUTPUT",
                            (os.environ.get("MANIFOLD_SAE_OUTPUT_DIR") or "runs") + "/dashboard.md"))
    args = parser.parse_args()

    if not args.checkpoint:
        print("[dash] --checkpoint or MSAE_CHECKPOINT is required", file=sys.stderr)
        return 2
    require_cuda_if_env()
