...
class QualiaManifold:
    ...
    def __init__(self, X, records, layer, axis_unit, k_pc, n_basis, degree,
        ...
        self.tn = tn
        # gamfit-built basis + penalty on the standardized latent.
        B, P = bspline_1d_basis(tn, n_basis=n_basis, degree=degree)   # (N,K),(K,K)
        self.B_coef, _ = reml_fit(B, Z, P)             # (K, k): coefficients per rep-PC
        # SAME knots bspline_1d_basis used (so dB/dt aligns with the fitted B).
        self.knots = _resolve_knots_tensor(
            torch.from_numpy(np.ascontiguousarray(tn)), n_basis, degree=degree)
        self.degree = degree
        # fit diagnostics
        fit = B @ self.B_coef
...
def main() -> None:
    ...
    D = X.shape[2]
    steer_layer = args.steer_layer if args.steer_layer is not None else int(round(0.40 * (X.shape[1] - 1)))
    stems = CLOZE_GROUPS["self"][:2] + CLOZE_GROUPS["self_1p"][:1]
    axis_unit = compute_qualia_axis(X, records, steer_layer)
    typ_norm = float(np.median(np.linalg.norm(X[:, steer_layer, :], axis=1)))
    print(f"[setup] X={X.shape} layer={steer_layer} D={D} typ_norm={typ_norm:.2f}", flush=True)

    # ---- fit the curved manifold g(t) with gamfit ----------------------------
    print("[fit] fitting qualia manifold g(t) via gamfit B-spline + REML ...", flush=True)
    man = QualiaManifold(X, records, steer_layer, axis_unit, args.k_pc,
                         args.n_basis, args.degree, bspline_1d_basis, reml_fit, fit_top_pcs)
    print(f"[fit] R2(g)={man.r2:.4f} R2(linear)={man.r2_linear:.4f} "
          f"curvature_gain={man.r2 - man.r2_linear:+.4f}", flush=True)
    print(f"[fit] tangent cos(t_lo,t_hi)={man.tangent_cos_lo_hi:+.3f} "
          f"(|<1| => tangent rotates along manifold); |tangent.axis|@mid={man.tangent_axis_cos:.3f}",
          flush=True)
    ...
    result: dict[str, Any] = {
        ...
        "D": int(D), "typ_resid_norm": typ_norm, "stems": stems,
        "manifold_fit": {
    ...
    # ---- LINEAR vs TANGENT at matched alphas (greedy + a sampled high) -------
    alphas = [-128, -64, -32, 0, 32, 64, 128]
    print("\n[cmp] LINEAR axis vs MANIFOLD TANGENT (greedy)", flush=True)
    result["linear_vs_tangent_greedy"] = []
    for alpha in alphas:
        ...
        tan = _steer_gen(_generate, model, tok, args.device, stems, layer_mod, "tangent",
                         man, axis_unit, alpha, typ_norm, D, torch, dtype, False, args.max_new_tokens)
        result["linear_vs_tangent_greedy"].append({
            "alpha": float(alpha),
    ...
    print("\n[cmp] sampled at +/-64,128", flush=True)
    result["linear_vs_tangent_sample"] = []
    for alpha in [-128, -64, 64, 128]:
        ...
        tan = _steer_gen(_generate, model, tok, args.device, stems, layer_mod, "tangent",
                         man, axis_unit, alpha, typ_norm, D, torch, dtype, True, args.max_new_tokens)
        result["linear_vs_tangent_sample"].append({
            "alpha": float(alpha),
            "linear": [{"stem": g["stem"], "gen": g["gen"]} for g in lin],
            "tangent": [{"stem": g["stem"], "gen": g["gen"]} for g in tan]})
        print(f"  alpha={alpha:+d} (sample)\n    LIN: {lin[0]['gen'][:78]!r}\n    TAN: {tan[0]['gen'][:78]!r}",
              flush=True)
